...
export const getStaticProps = async () => {
  ...
  const ARTICLES_DIR = path.join(process.cwd(), "pages/articles/concurrency");
  ...
  const articles = articlesPaths.map((articlePath) => {
    ...
    const source = fs.readFileSync(
      path.join(process.cwd(), "pages/articles/concurrency", articlePath),
      "utf8"
    );
...
const CONC_Articles = ({ articles }) => {
  return (
    <>
      <Head>
        <title>Concurrency</title>
        <meta name="description" content="View all my articles about concurrency" />
      </Head>
      <section>
        <header className="articles-header">
          <div className="wrapper">
            <h1 className="font-extrabold text-5xl">
              View all my articles about concurrency
            </h1>
          </div>
        </header>
        <ul className="articles">
          {articles.map((article) => (
            <li key={article.slug} className="article">
              <Link href={`concurrency/${article.slug}`}>
...
export default CONC_Articles;

// define layout for articles page
CONC_Articles.getLayout = (page) => {

...
export const getStaticProps = async () => {
  ...
  const ARTICLES_DIR = path.join(process.cwd(), "pages/articles/ai-ml");
  ...
  const articles = articlesPaths.map((articlePath) => {
    ...
    const source = fs.readFileSync(
      path.join(process.cwd(), "pages/articles/ai-ml", articlePath),
      "utf8"
    );
...
const AI_ML_Articles = ({ articles }) => {
  return (
    <>
      <Head>
        <title>AI/ML</title>
        <meta name="description" content="View all my articles about AI/ML" />
      </Head>
      <section>
        <header className="articles-header">
          <div className="wrapper">
            <h1 className="font-extrabold text-5xl">
              View all my articles about AI/ML
            </h1>
          </div>
        </header>
...
export default AI_ML_Articles;

// define layout for articles page
AI_ML_Articles.getLayout = (page) => {

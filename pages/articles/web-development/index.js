...
export const getStaticProps = async () => {
  ...
  const ARTICLES_DIR = path.join(process.cwd(), "pages/articles/web-development");
  ...
  const articles = articlesPaths.map((articlePath) => {
    ...
    const source = fs.readFileSync(
      path.join(process.cwd(), "pages/articles/web-development", articlePath),
      "utf8"
    );
...
const WEB_DEV_Articles = ({ articles }) => {
  return (
    <>
      <Head>
        <title>Web Development</title>
        <meta name="description" content="View all my articles about web-development" />
      </Head>
      <section>
        <header className="articles-header">
          <div className="wrapper">
            <h1 className="font-extrabold text-5xl">
              View all my articles about web development
            </h1>
          </div>
        </header>
        <ul className="articles">
          {articles.map((article) => (
            <li key={article.slug} className="article">
              <Link href={`web-development/${article.slug}`}>
...
export default WEB_DEV_Articles;

// define layout for articles page
WEB_DEV_Articles.getLayout = (page) => {
